...
class SubscriptionManager:
    ...
    def check_usage_limits(
        self, user_id: str, resource_type: str, amount: int = 1
    ) -> bool:
    ...
    def get_subscription_metrics(self) -> Dict[str, Any]:
        ...
        for tier in SubscriptionTier:
            count = len(
                [
                    ...
                    if s.plan.tier == tier and s.is_active
                ]
            )
            tier_distribution[tier.value] = count

        total_revenue_potential = sum(
            self._get_tier_price(s.plan.tier)
            for s in self.subscriptions.values()
            if s.is_active
        )
        ...
        return {
            ...
            "premium_adoption_rate": (
                (
                    tier_distribution.get("premium", 0)
                    + tier_distribution.get("enterprise", 0)
                )
                / active_users
                if active_users > 0
                else 0
