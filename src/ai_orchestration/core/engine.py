...
class StrategicEngine:
    ...
    def __init__(self):
        ...
        self.learning_weights = np.array(
            [0.4, 0.3, 0.2, 0.1]
        )  # [performance, load, subscription, health]
        self._running = False
    ...
    async def _decide_agent_deployment(
        self, context: StrategicContext
    ) -> StrategicDecision:
        ...
        optimal_agents = max(
            base_agents, int(base_agents + load_factor * 5 + premium_users * 0.1)
        )
    ...
    async def _decide_emergent_adaptation(
        self, context: StrategicContext
    ) -> StrategicDecision:
        ...
        if adaptation_needed:
            ...
            reasoning = (
                f"Emergent adaptation triggered by anomaly score {anomaly_score:.3f}"
            )
        else:
            adaptation = {
    ...
    def _calculate_confidence(
        self, context: StrategicContext, decision_type: str
    ) -> float:
        ...
        confidence = (
            data_quality * 0.3 + system_stability * 0.4 + historical_success * 0.3
        )
    ...
    def _detect_anomalies(self, context: StrategicContext) -> float:
        ...
        historical_std = (
            np.std(context.historical_performance)
            if len(context.historical_performance) > 1
            else 0.1
        )

        # Z-score based anomaly detection
        z_score = (
            abs(current_perf - historical_mean) / historical_std
            if historical_std > 0
            else 0
        )
    ...
    def _get_historical_success_rate(self, decision_type: str) -> float:
        ...
        relevant_decisions = [
            d for d in self.decision_history if d.decision_type.value == decision_type
        ]
    ...
    async def _update_learning_weights(self) -> None:
        ...
        self.logger.debug(
            "Updated learning weights", weights=self.learning_weights.tolist()
        )
