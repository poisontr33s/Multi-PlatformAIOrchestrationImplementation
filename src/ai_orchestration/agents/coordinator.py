...
class AgentCoordinator:
    ...
    async def assign_task(
        ...
        suitable_agents = self._find_suitable_agents(
            required_capabilities, subscription_tier
        )

        if not suitable_agents:
            # Try to deploy a new agent if possible
            agent_type = self._determine_agent_type_for_capabilities(
                required_capabilities
            )
            if agent_type:
                agent_id = await self.deploy_agent(
                    agent_type, required_capabilities, subscription_tier
                )
    ...
    async def complete_task(
        ...
        if success:
            ...
            agent.metrics.success_rate = (
                agent.metrics.tasks_completed / total_tasks if total_tasks > 0 else 0.0
            )
        else:
        ...
        self.logger.debug(
            "Task completed", agent_id=agent_id, task_id=task_id, success=success
        )
    ...
    def get_coordination_metrics(self) -> Dict[str, Any]:
        ...
        pool_sizes = {
            pool_name: len(agent_set)
            for pool_name, agent_set in self.agent_pools.items()
        }
        ...
        if self.agents:
            ...
            avg_utilization = np.mean(
                [a.metrics.resource_utilization for a in self.agents.values()]
            )
    ...
    async def _deploy_initial_agents(self) -> None:
        ...
        await self.deploy_agent(
            AgentType.STRATEGIC_ADVISOR, subscription_tier="premium"
        )
    ...
    def _find_suitable_agents(
        ...
        for agent in self.agents.values():
            ...
            if not self._check_tier_compatibility(
                agent.subscription_tier_required, subscription_tier
            ):
                continue
    ...
    async def _select_agent(
        ...
        if self.coordination_strategy == CoordinationStrategy.PRIORITY_WEIGHTED:
    ...
    async def _check_agent_health(self) -> None:
        ...
        for agent in self.agents.values():
            ...
            if (
                ...
                and (current_time - agent.metrics.last_activity).seconds > 3600
            ):  # 1 hour
    ...
    async def _update_agent_metrics(self) -> None:
        ...
        for agent in self.agents.values():
            ...
            if agent.status == AgentStatus.BUSY:
                agent.metrics.resource_utilization = min(
                    1.0, agent.metrics.resource_utilization + 0.1
                )
            elif agent.status == AgentStatus.IDLE:
                agent.metrics.resource_utilization = max(
                    0.0, agent.metrics.resource_utilization - 0.05
                )
    ...
    async def _scale_agent_fleet(self) -> None:
        ...
        # Scale down if too many idle agents
            ...
            if self.agent_pools["idle"]:
                ...
                await self.terminate_agent(agent_id)
                self.logger.info(
                    "Scaled down agent fleet", total_agents=len(self.agents)
                )
